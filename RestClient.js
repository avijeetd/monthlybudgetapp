...
// Model for Expenses entity
    models.Expenses = Backbone.Model.extend({
        urlRoot: "http://localhost:8282/monthlybudgetservices/webresources/expenses/",
        idAttribute: 'id',
        defaults: {
            amount: "",
            description: ""
        ...
        sync: function(method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8282/monthlybudgetservices/webresources/expenses/';
            }
            if (method == 'update') {
                options.url = 'http://localhost:8282/monthlybudgetservices/webresources/expenses/';
            }
    ...
    // Collection class for Expenses entities
    models.ExpensesCollection = Backbone.Collection.extend({
        model: models.Expenses,
        url: "http://localhost:8282/monthlybudgetservices/webresources/expenses/",
        sync: function(method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function(jqXHR, textStatus, errorThrown) {
...
$(function() {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function() {
            this.collection = new models.ExpensesCollection();
            var self = this;
            this.collection.fetch({
                success: function() {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-expenses-list-itemis template identifier for item
                        templateName: '#tpl-expenses-list-item'
                    });
        ...
        details: function(id) {
            if (this.collection) {
                this.expenses = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.expenses,
                    // tpl-expenses-details is template identifier for chosen model element
                    templateName: '#tpl-expenses-details',
        ...
        create: function() {
            ...
            var self = this;
            var dataModel = new models.Expenses();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-expenses-details is a template identifier for chosen model element
                templateName: '#tpl-expenses-details',
